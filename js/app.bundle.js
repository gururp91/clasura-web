 ...
 * Generated at: 2026-09-24T11:36:33.413Z
 * Version: 20260924-113633
 * Modules: 22
 */
...
var _initialOddsCache = null;
function getInitialOddsCache() {
  if (_initialOddsCache) return _initialOddsCache;
  try {
    var raw = localStorage.getItem('clasura_initial_odds');
    _initialOddsCache = raw ? JSON.parse(raw) : {};
  } catch(e) {
    _initialOddsCache = {};
  }
  return _initialOddsCache;
}
function getStoredInitialOdd(id) {
  if (!id) return null;
  var cache = getInitialOddsCache();
  var v = cache[String(id)];
  return (v != null && !isNaN(Number(v))) ? Number(v) : null;
}
function storeInitialOdd(id, odd) {
  if (!id || odd == null || isNaN(Number(odd))) return;
  var cache = getInitialOddsCache();
  if (cache[String(id)] == null) {
    cache[String(id)] = Number(odd);
    try {
      localStorage.setItem('clasura_initial_odds', JSON.stringify(cache));
    } catch(e) {}
  }
}
window.getStoredInitialOdd = getStoredInitialOdd;
window.storeInitialOdd = storeInitialOdd;

var SUPABASE_HAS_INITIAL_ODD = false;
function checkSupabaseInitialOddCol() {
  sbFetch('proposals?select=initial_odd&limit=1').then(function() {
    SUPABASE_HAS_INITIAL_ODD = true;
    window.SUPABASE_HAS_INITIAL_ODD = true;
  }).catch(function() {
    SUPABASE_HAS_INITIAL_ODD = false;
    window.SUPABASE_HAS_INITIAL_ODD = false;
  });
}
checkSupabaseInitialOddCol();
...
function mapProposal(row,voteMap){
  ...
  var curOdd = Number(row.odd) || 1;
  var initOdd = (row.initial_odd != null && !isNaN(Number(row.initial_odd)))
    ? Number(row.initial_odd)
    : (getStoredInitialOdd(row.id) || curOdd);
  storeInitialOdd(row.id, initOdd);
  return{
    id:row.id,date:row.date,by:row.by_name,also:row.also||[],
    createdAt:row.created_at?new Date(row.created_at).getTime():0,
    match:row.match,league:row.league||'',ko:row.ko,cls:row.cls,
    market:row.market,pick:row.pick,odd:curOdd,
    initialOdd:initOdd,
...
function mapCoupon(row){
  ...
  var isSys=String(row.type||'').startsWith('sistem');
  var sysObj=null;
  if(isSys){
    var parts=String(row.type).split(':');
    var lvls=parts[1]?parts[1].split(',').map(Number).filter(Boolean):[];
    var mult=parts[2]?Number(parts[2]):1;
    sysObj={isSystem:true,levels:lvls,multiplier:mult};
  }
  return{
...
function proposalToRow(p){
  var initOdd = Number(p.initialOdd != null ? p.initialOdd : p.odd) || 1;
  storeInitialOdd(p.id, initOdd);
  var row = {
  ...
  if (SUPABASE_HAS_INITIAL_ODD) {
    row.initial_odd = initOdd;
  }
  return row;
}
...
function mutate(action,payload){
  ...
  if(action==='addProposal'){
    ...
    var pj={
      ...
      initialOdd:Number(payload.initialOdd != null ? payload.initialOdd : payload.odd) || 1,
      eventId:row.event_id,mktI:row.mkt_i,no:row.no,
      comment:row.comment||''
    };
...
function getProposalComments(p){
  if(!p||!p.comment)return [];
  var raw=p.comment;
  if(Array.isArray(raw))return raw;
  if(typeof raw==='string'){
    var s=raw.trim();
    if(s.charCodeAt(0)===91&&s.charCodeAt(s.length-1)===93){
      try{
...
if(typeof window!=='undefined'){ window.getProposalComments = getProposalComments; }
...
function renderProps(){
  ...
  var propHtml=function(p){
    ...
    var cmts=getProposalComments(p);
    var cmtCount=cmts.length;
    var hasCmt=cmtCount>0;
    var latestCmt=hasCmt?cmts[cmts.length-1]:null;
    var cmtPreview=latestCmt?(latestCmt.by?('<b>'+esc(latestCmt.by)+':</b> '):'')+esc(latestCmt.text):'';
    var moreCmtBadge=cmtCount>1?('<span class="cmt-more-pill">+'+(cmtCount-1)+'</span>'):'';
    var curOdd = numOdd(p.odd) || 1;
    var initOdd = numOdd(p.initialOdd != null ? p.initialOdd : p.initial_odd);
    if (!initOdd || isNaN(initOdd)) {
      if (typeof getStoredInitialOdd === 'function') {
        initOdd = getStoredInitialOdd(p.id);
      }
    }
    if (!initOdd || isNaN(initOdd)) initOdd = curOdd;
    
    var oddDiff = Math.round((curOdd - initOdd) * 100) / 100;
    var oddDiffHtml = '';
    if (Math.abs(oddDiff) >= 0.01) {
      if (curOdd > initOdd) {
        oddDiffHtml = '<div class="prop-odd-diff up" title="İlk eklenen oran: @' + fmtOdd(initOdd) + ' → Güncel oran: @' + fmtOdd(curOdd) + ' (+' + fmtOdd(oddDiff) + ')">' +
          '<span class="odd-diff-label">İlk:</span><span class="odd-diff-num">@' + fmtOdd(initOdd) + '</span><span class="odd-diff-icon">↗</span>' +
        '</div>';
      } else {
        oddDiffHtml = '<div class="prop-odd-diff down" title="İlk eklenen oran: @' + fmtOdd(initOdd) + ' → Güncel oran: @' + fmtOdd(curOdd) + ' (' + fmtOdd(oddDiff) + ')">' +
          '<span class="odd-diff-label">İlk:</span><span class="odd-diff-num">@' + fmtOdd(initOdd) + '</span><span class="odd-diff-icon">↘</span>' +
        '</div>';
      }
    } else {
      oddDiffHtml = '<div class="prop-odd-diff same" title="İlk eklenen oran: @' + fmtOdd(initOdd) + ' (oran değişmedi)">' +
        '<span class="odd-diff-label">İlk:</span><span class="odd-diff-num">@' + fmtOdd(initOdd) + '</span>' +
      '</div>';
    }
    ...
    return '<div class="prop" data-id="'+p.id+'">'+
      '<div class="prop-top">'+
        '<div class="prop-title"><b>'+esc(p.match)+'</b>'+(badge?'<div class="prop-badges">'+badge+'</div>':'')+
          ...
          (hasCmt?'<div class="prop-comment-box" data-act="comment" title="Yorumları Gör / Yanıtla"><span class="cmt-icon">💬</span><span class="cmt-txt">'+cmtPreview+'</span>'+moreCmtBadge+'</div>':'')+
        '</div>'+
        '<div class="prop-odd"><span class="podd" title="Güncel Oran">@'+fmtOdd(curOdd)+'</span>'+oddDiffHtml+'<span class="pill '+oc.k+' prop-odd-pill">'+oc.t+'</span></div>'+
      '</div>'+
      '<div class="prop-foot">'+
        ...
        '</div>'+
        '<div class="prop-btns">'+
          '<button class="btn tiny icon-only'+(hasCmt?' has-comment':'')+'" data-act="comment" title="'+(hasCmt?cmtCount+' yorum · Yanıtla':'Yorum ekle')+'">'+
            '<ion-icon name="'+(hasCmt?'chatbubble':'chatbubble-outline')+'"></ion-icon>'+
            (hasCmt?'<span class="cmt-count-bubble">'+cmtCount+'</span>':'')+
          '</button>'+
  ...
  el.querySelectorAll('.prop').forEach(function(box){
    ...
    box.querySelectorAll('[data-act=comment]').forEach(function(cmtEl){
      cmtEl.onclick=function(ev){
        if(ev){ev.preventDefault();ev.stopPropagation();}
        openProposalComment(p);
      };
    });
...
var LEAGUE_ALLOW=[
  ...
  {label:'Trendyol 1. Lig', test:function(n){return /(türkiye|tff|trendyol|ptt).*(1\.\s*lig|birinci\s*lig)|^1\.\s*lig$/i.test(n)&&!/kadın|women|frauen|bayan|u1[89]|u2[01]|kupa|cup/i.test(n);}},
  ...
  {label:'İsviçre', test:function(n){return /(isviçre|İsviçre|swiss|switzerland).*(süper\s*lig|super\s*league|1\.\s*lig)|isviçre\s*ligi/i.test(n)&&!/2\.\s*lig|kadın|women|frauen|bayan|challenge|cup|kupa|u1[89]|u2[01]/i.test(n);}},
  ...
  {label:'Uluslar Ligi', test:function(n){return /(uefa.*)?(uluslar\s*ligi|nations\s*league)/i.test(n)&&!/kadın|women|frauen|bayan|voleybol|volleyball|basket|u1[89]|u2[01]/i.test(n);}}
];
var LEAGUE_ID_HINTS={
...
function registerDailyComps(compsMap){
  for(var id in compsMap){
    ...
    else if(/uluslar|nations/i.test(n)&&!LEAGUE_ID_HINTS[ci])LEAGUE_ID_HINTS[ci]='Uluslar Ligi';
...
function getLeagueAllow(label){
  for(var i=0;i<LEAGUE_ALLOW.length;i++){if(LEAGUE_ALLOW[i].label===label||LEAGUE_ALLOW[i].test(label))return LEAGUE_ALLOW[i];}
  return null;
}
...
function syncProposalOddsFromLive(opts){
  ...
  return chain.then(function(){
    ...
    props.forEach(function(p){
      ...
      var initOdd = Number(p.initialOdd != null ? p.initialOdd : p.odd) || Number(p.odd) || 1;
      p.initialOdd = initOdd;
      p.odd = live;
      if (typeof storeInitialOdd === 'function') {
        storeInitialOdd(p.id, initOdd);
      }
      var patchBody = { odd: live };
      if (typeof SUPABASE_HAS_INITIAL_ODD !== 'undefined' && SUPABASE_HAS_INITIAL_ODD) {
        patchBody.initial_odd = initOdd;
      }
      updates.push({ id: String(p.id), body: patchBody });
    });
    if(!updates.length){
...
function teamFormBlockHtml(data) {
  ...
  function renderFormPills(form) {
    ...
    // Kullanıcı tercihi: Son maç en sağda olsun, sola doğru geriye gelsin
    var list = form.slice().reverse();
    return list.map(function(item, idx) {
      ...
      var title = (isLatest ? '⭐ SON MAÇ: ' : '') + (item.opp ? item.opp + where + ': ' : '') + (item.score ? item.score : '') + (item.date ? ' (' + item.date + ')' : '');
      var scoreTxt = item.score ? '<span style="font-size:9px;font-weight:800;opacity:0.9;margin-left:3px;">' + esc(item.score) + '</span>' : '';
      var latestStyle = isLatest ? 'border:1.5px solid #ffffff;box-shadow:0 0 6px rgba(255,255,255,0.45);' : 'border:1px solid transparent;';
      return '<span title="' + esc(title) + '" style="display:inline-flex;align-items:center;background:' + bg + ';color:#070d1a;-webkit-text-fill-color:#070d1a;font-weight:900;font-size:11px;padding:2.5px 6px;border-radius:5px;cursor:help;white-space:nowrap;' + latestStyle + '">' + label + scoreTxt + '</span>';
    }).join('');
  }
  ...
  return '<div class="team-form-dashboard" style="background:#0c1736;border:1px solid #ff7a1a;border-radius:14px;padding:14px;margin-bottom:14px;box-shadow:0 8px 24px rgba(0,0,0,0.35);">' +
    ...
    '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">' +
      '<div style="font-size:11px;font-weight:800;color:#ffb92e;-webkit-text-fill-color:#ffb92e;text-transform:uppercase;letter-spacing:0.5px;">📈 Son Maçlar (Form &amp; Skorlar)</div>' +
      '<div style="font-size:10px;color:#a8b8db;font-weight:600;">Eski ➔ <span style="color:#3dd68c;font-weight:800;">Son Maç (Sağda)</span></div>' +
    '</div>' +
...
function formatCommentTime(ts){
  ...
  if(d.toDateString()===yesterday.toDateString())return 'Dün '+timeStr;
...
function openProposalComment(p){
  ...
  var getCmts=(typeof window.getProposalComments==='function')
    ? window.getProposalComments
    : (typeof getProposalComments==='function'?getProposalComments:function(){return [];});
  var cmts=getCmts(p).slice();
  ...
  function renderThreadHtml(list){
    if(!list||!list.length){
      return '<div class="cs-empty"><ion-icon name="chatbubbles-outline"></ion-icon><p>Henüz yorum yapılmamış.<br>İlk yorumu sen ekle!</p></div>';
    }
    return list.map(function(c){
      var isMine=!!(ME&&ME.name&&c.by===ME.name);
      var isProposer=(c.by&&(c.by===p.by||(p.also&&p.also.indexOf(c.by)>=0)));
      var time=formatCommentTime(c.at);
      return '<div class="comment-item'+(isMine?' mine':'')+'" data-cid="'+esc(c.id||'')+'">'+
        '<div class="comment-header">'+
          '<span class="comment-author">'+esc(c.by||'Üye')+'</span>'+
          (isProposer?'<span class="badge-proposer">Öneren</span>':'')+
          (time?'<span class="comment-time">'+esc(time)+'</span>':'')+
          (isMine?'<button type="button" class="btn-del-cmt" data-cid="'+esc(c.id||'')+'" title="Yorumu sil"><ion-icon name="trash-outline"></ion-icon></button>':'')+
        '</div>'+
  ...
  box.innerHTML='<div class="modal-shell comment-sheet">'+
    '<div class="modal-head"><div class="modal-head-text">'+
      '<h3 id="propCommentHeadTitle">Yorumlar ('+cmts.length+')</h3>'+
      '<p class="modal-sub">Grup tartışması & değerlendirme</p>'+
    '</div>'+
    '<button type="button" class="modal-x" id="mX" aria-label="Kapat"><ion-icon name="close-outline"></ion-icon></button></div>'+
    '<div class="modal-body" style="display:flex;flex-direction:column;min-height:0;flex:1;">'+
      ...
      '</div>'+
      '<div class="comment-thread" id="propCommentList">'+
        renderThreadHtml(cmts)+
      '</div>'+
    '</div>'+
    '<div class="modal-foot comment-foot">'+
      '<form id="propCommentForm" class="comment-input-bar">'+
        '<input type="text" id="propCommentInput" maxlength="280" placeholder="Yorum yaz (Enter ile gönder)…" autocomplete="off" />'+
        '<button type="submit" class="btn primary btn-send-cmt" id="btnSendComment" title="Gönder">'+
  ...
  var threadEl=document.getElementById('propCommentList');
  if(threadEl)threadEl.scrollTop=threadEl.scrollHeight;

  function bindDelBtns(){
    var listEl=document.getElementById('propCommentList');
    if(!listEl)return;
    listEl.querySelectorAll('.btn-del-cmt').forEach(function(btn){
      btn.onclick=function(ev){
        if(ev){ev.preventDefault();ev.stopPropagation();}
        var cid=this.getAttribute('data-cid');
        if(!cid)return;
        cmts=cmts.filter(function(c){return String(c.id)!==String(cid);});
        saveAndSync();
        toast('Yorum silindi');
      };
    });
  ...
  function saveAndSync(){
    var serialized=cmts.length?JSON.stringify(cmts):'';
    p.comment=serialized;
    if(typeof S!=='undefined'&&S.proposals){
      var target=S.proposals.find(function(x){return String(x.id)===String(p.id);});
      if(target)target.comment=serialized;
    }
    if(typeof renderProps==='function'){
      try{renderProps();}catch(e){}
    }
    var headTitle=document.getElementById('propCommentHeadTitle');
    if(headTitle)headTitle.textContent='Yorumlar ('+cmts.length+')';

    var listEl=document.getElementById('propCommentList');
    if(listEl){
      listEl.innerHTML=renderThreadHtml(cmts);
      bindDelBtns();
      listEl.scrollTop=listEl.scrollHeight;
    }

    if(typeof mutate==='function'){
      mutate('setProposalComment',{id:p.id,comment:serialized}).catch(function(err){
        console.error('[CLASURA] setProposalComment error:',err);
        toast('Yorum kaydedilemedi!');
      });
    }
  }
  ...
  var form=document.getElementById('propCommentForm');
  if(form){
...
function renderSwipeDeck(){
  ...
  var when=p.ko?new Date(p.ko*1000).toLocaleString('tr-TR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}):'';
  var cmts=(typeof window.getProposalComments==='function')?window.getProposalComments(p):(typeof getProposalComments==='function'?getProposalComments(p):[]);
  var latestCmt=cmts.length?cmts[cmts.length-1]:null;
  var cmtPreview=latestCmt?((latestCmt.by?esc(latestCmt.by)+': ':'')+esc(latestCmt.text)):'';
    var curOdd=Number(p.odd)||1;
    var initOdd=Number(p.initialOdd!=null?p.initialOdd:p.initial_odd);
    if(!initOdd||isNaN(initOdd)){if(typeof getStoredInitialOdd==='function')initOdd=getStoredInitialOdd(p.id);}
    if(!initOdd||isNaN(initOdd))initOdd=curOdd;
    var diff=Math.round((curOdd-initOdd)*100)/100;
    var initOddHtml=(Math.abs(diff)>=0.01)
      ?(' <span style="font-size:12px;color:#94a3b8;font-weight:600;margin-left:4px;" title="İlk eklenen oran: @'+fmtOdd(initOdd)+'">(İlk: @'+fmtOdd(initOdd)+(curOdd>initOdd?' ↗':' ↘')+')</span>')
      :'';
    ...
    (cmtPreview?'<p class="sc-pick" style="font-style:italic;color:#8fa2c7;font-size:12px;">💬 “'+cmtPreview+'”'+(cmts.length>1?' <span style="font-size:10px;background:rgba(255,122,26,0.2);color:#ff9d42;padding:1px 5px;border-radius:6px;">+'+(cmts.length-1)+'</span>':'')+'</p>':'')+
    '<div class="sc-row"><div><div class="sc-odd">@'+fmtOdd(curOdd)+initOddHtml+'</div>'+
...
function basketSelHtml(s){
  ...
  var isBanko=!!s.banko;
  return '<div class="bsel '+(isBanko?'is-banko':'')+'" data-id="'+s.id+'">'+
    '<button type="button" class="bsel-banko-btn '+(isBanko?'on':'')+'" data-act="banko" title="Banko Seç (B)">B</button>'+
    '<div class="bsel-main"><b>'+(isBanko?'<span class="banko-tag">B</span>':'')+esc(s.match)+'</b><small>'+meta+'</small></div>'+
...
if(!APP.basketMode) APP.basketMode='normal';
if(!APP.basketSystemLevels) APP.basketSystemLevels=[];
if(!APP.basketMultiplier) APP.basketMultiplier=1;
...
function renderBasket(){
  ...
  if(el){
    if(!sels.length){
      ...
      el.querySelectorAll('.bsel').forEach(function(row){
        ...
        var bBtn=row.querySelector('.bsel-banko-btn');
        if(bBtn){
          bBtn.onclick=function(e){
            e.stopPropagation();
            var target=basket().filter(function(x){return String(x.id)===String(row.dataset.id);})[0];
            if(target){
              target.banko=!target.banko;
              saveBasket();renderBasket();
            }
          };
        }
  ...
  var isSys=APP.basketMode==='system';
  var bCard=document.getElementById('basketCard')||document.querySelector('.basket-card');
  if(bCard)bCard.classList.toggle('mode-system',isSys);
  var btnNorm=document.getElementById('btnModeNormal');
  var btnSys=document.getElementById('btnModeSystem');
  if(btnNorm)btnNorm.classList.toggle('on',!isSys);
  if(btnSys)btnSys.classList.toggle('on',isSys);

  var sysBox=document.getElementById('basketSystemBox');
  if(sysBox)sysBox.style.display=isSys?'block':'none';

  var stakeEl=document.getElementById('bStake');
  var bTotalOddsEl=document.getElementById('bTotalOdds');
  var bOddsLabel=document.getElementById('bOddsLabel');
  var bPotentialEl=document.getElementById('bPotential');

  if(isSys){
    var bankos=sels.filter(function(x){return !!x.banko;});
    var nonBankos=sels.filter(function(x){return !x.banko;});
    var N=sels.length, B=bankos.length, M=nonBankos.length;

    var sysLevelsEl=document.getElementById('bSystemLevels');
    var sysHint=document.getElementById('bSysHint');
    var sysColCount=document.getElementById('bSysColCount');
    var sysTotalStake=document.getElementById('bSysTotalStake');
    var multInput=document.getElementById('bSysMultiplier');
    var mult=Math.max(1, parseInt(multInput?multInput.value:1, 10)||1);
    APP.basketMultiplier=mult;

    if(N<2){
      if(sysHint)sysHint.textContent='Sistem için en az 2 maç ekleyin';
      if(sysLevelsEl)sysLevelsEl.innerHTML='<div class="muted" style="padding:4px;font-size:11.5px">En az 2 maç seçilmelidir.</div>';
      if(sysColCount)sysColCount.textContent='0 Kolon';
      if(sysTotalStake)sysTotalStake.textContent='0 TL';
      if(bOddsLabel)bOddsLabel.textContent='Sistem';
      if(bTotalOddsEl)bTotalOddsEl.textContent='–';
      if(bPotentialEl)bPotentialEl.textContent='0 TL';
      return;
    }

    if(B===N){
      if(sysHint)sysHint.textContent='Tüm maçlar banko olamaz (en az 1 normal maç)';
      if(sysLevelsEl)sysLevelsEl.innerHTML='<div class="muted" style="padding:4px;font-size:11.5px">Kombinasyon için en az 1 maç normal kalmalıdır.</div>';
      if(sysColCount)sysColCount.textContent='0 Kolon';
      if(sysTotalStake)sysTotalStake.textContent='0 TL';
      if(bOddsLabel)bOddsLabel.textContent='Sistem';
      if(bTotalOddsEl)bTotalOddsEl.textContent='–';
    ...
    // Geçerli sistem seviyeleri: B + 1'den N'ye kadar
    var validLevels=[];
    for(var lvl=Math.max(1, B+1); lvl<=N; lvl++){
      validLevels.push(lvl);
    }

    // Seçili seviyeleri geçerlilerle sınırla
    APP.basketSystemLevels=(APP.basketSystemLevels||[]).filter(function(x){return validLevels.indexOf(x)>=0;});
    if(!APP.basketSystemLevels.length){
      // Varsayılan: N veya (N-1 ve N)
      if(N>B+1 && validLevels.indexOf(N-1)>=0){
        APP.basketSystemLevels=[N-1, N];
      }else{
        APP.basketSystemLevels=[N];
      }
    }

    if(sysHint){
      sysHint.textContent=B>0?(B+' Banko · '+M+' Normal Maç'):'Banko için [B] butonunu kullan';
    }

    if(sysLevelsEl){
      sysLevelsEl.innerHTML=validLevels.map(function(lvl){
        var k=lvl-B;
        var cols=mathChoose(M, k);
        var isOn=APP.basketSystemLevels.indexOf(lvl)>=0;
        return '<button type="button" class="system-level-chip '+(isOn?'on':'')+'" data-lvl="'+lvl+'">'+
          'Sistem '+lvl+'<small style="display:block;font-size:9.5px;font-weight:600;opacity:.85">('+cols+' kol)</small></button>';
      }).join('');

      sysLevelsEl.querySelectorAll('.system-level-chip').forEach(function(btn){
        btn.onclick=function(){
          var l=Number(btn.dataset.lvl);
          var idx=APP.basketSystemLevels.indexOf(l);
          if(idx>=0){
            if(APP.basketSystemLevels.length>1){
              APP.basketSystemLevels.splice(idx,1);
            }else{
              toast('En az bir sistem seviyesi seçili olmalıdır');
              return;
            }
          }else{
            APP.basketSystemLevels.push(l);
          }
          APP.basketSystemLevels.sort(function(a,b){return a-b;});
          renderBasket();
        };
      });
    }
    ...
    var totalCols=0;
    APP.basketSystemLevels.forEach(function(lvl){
      totalCols+=mathChoose(M, lvl-B);
    });

    var totalStake=totalCols*mult;
    if(sysColCount)sysColCount.textContent=totalCols+' Kolon';
    if(sysTotalStake)sysTotalStake.textContent=fmtTL(totalStake);
    if(stakeEl)stakeEl.value=totalStake;

    var tempCoupon={
      isSystem:true,
      type:'sistem:'+APP.basketSystemLevels.join(',')+':'+mult,
      stake:totalStake,
      selections:sels,
      system:{levels:APP.basketSystemLevels, multiplier:mult}
    };
    var pot=couponPotentialWin(tempCoupon);

    if(bOddsLabel)bOddsLabel.textContent='Sistem ('+totalCols+' Kolon)';
    if(bTotalOddsEl)bTotalOddsEl.textContent=APP.basketSystemLevels.map(function(x){return 'S'+x;}).join('+');
    if(bPotentialEl)bPotentialEl.textContent='Min: '+fmtTL(pot.min)+' · Maks: '+fmtTL(pot.max);

  }else{
    if(bOddsLabel)bOddsLabel.textContent='Toplam oran';
...
var btnModeNormalEl=document.getElementById('btnModeNormal');
if(btnModeNormalEl){
  btnModeNormalEl.onclick=function(){
    APP.basketMode='normal';
    renderBasket();
  };
}
var btnModeSystemEl=document.getElementById('btnModeSystem');
if(btnModeSystemEl){
  btnModeSystemEl.onclick=function(){
    APP.basketMode='system';
    renderBasket();
  };
}
var bSysMultiplierEl=document.getElementById('bSysMultiplier');
if(bSysMultiplierEl){
  bSysMultiplierEl.oninput=function(){
    APP.basketMultiplier=Math.max(1, parseInt(bSysMultiplierEl.value, 10)||1);
    renderBasket();
  };
}
  ...
  sels.forEach(function(s){
    chain=chain.then(function(){
      ...
      var p={id:uid(),date:todayStr(),by:ME.name,createdAt:Date.now(),votes:[ME.name],
        match:s.match,league:s.league,ko:s.ko,cls:s.cls||null,market:s.market,pick:s.pick,odd:s.odd,initialOdd:s.odd,eventId:s.eventId,mktI:s.mktI,no:s.no,
...
document.getElementById('btnSaveCoupon').onclick=function(){
  ...
  var isSys=APP.basketMode==='system';
  if(isSys){
    if(sels.length<2){toast('Sistem kuponu için en az 2 maç ekleyin');return;}
    var bankos=sels.filter(function(x){return !!x.banko;});
    if(bankos.length===sels.length){toast('Tüm maçlar banko olamaz (en az 1 normal maç)');return;}
    if(!APP.basketSystemLevels||!APP.basketSystemLevels.length){toast('Lütfen en az bir sistem seviyesi seçin');return;}
    
    var mult=Math.max(1, parseInt(document.getElementById('bSysMultiplier')?document.getElementById('bSysMultiplier').value:1, 10)||1);
    var B=bankos.length, M=sels.length-B;
    var totalCols=0;
    APP.basketSystemLevels.forEach(function(lvl){
      totalCols+=mathChoose(M, lvl-B);
    });
    if(totalCols<=0){toast('Geçersiz sistem kombinasyonu');return;}
    var stake=totalCols*mult;

    var activeLevels=APP.basketSystemLevels.slice().sort(function(a,b){return a-b;});
    var couponType='sistem:'+activeLevels.join(',')+':'+mult;

    var c={
      id:uid(),
      date:document.getElementById('bDate').value||todayStr(),
      type:couponType,
      stake:stake,
      override:null,
      createdAt:Date.now(),
    ...
    mutate('saveCoupon',c).then(function(){
      ...
      toast('Sistem kuponu oluşturuldu ('+totalCols+' Kolon) · '+fmtTL(stake)+' kasadan düştü');
      switchTab('coupons');
    });
    return;
  }
  ...
  var c={id:uid(),date:document.getElementById('bDate').value||todayStr(),type:document.getElementById('bType').value,
    ...
    selections:sels.map(function(s){
      var x=JSON.parse(JSON.stringify(s));x.result='open';x.banko=false;
      var names=selectionProposers(s);
      if(names.length){x.by=names[0];x.also=names.slice(1);}
      return x;
    })};
...
function basketText(){
  ...
  if(isSys){
    var bankos=sels.filter(function(x){return !!x.banko;});
    var nonBankos=sels.filter(function(x){return !x.banko;});
    var B=bankos.length, M=nonBankos.length;
    var activeLevels=(APP.basketSystemLevels||[]).slice().sort(function(a,b){return a-b;});
    var mult=APP.basketMultiplier||1;
    var totalCols=0;
    activeLevels.forEach(function(lvl){ totalCols+=mathChoose(M, lvl-B); });
    var totalStake=totalCols*mult;
    var tempC={
      isSystem:true,type:'sistem:'+activeLevels.join(',')+':'+mult,stake:totalStake,selections:sels,
      system:{levels:activeLevels, multiplier:mult}
    };
    var pot=couponPotentialWin(tempC);

    return '🎲 SİSTEM KUPONU ('+activeLevels.join(',')+') – '+new Date().toLocaleDateString('tr-TR',{day:'numeric',month:'long'})+'\n'+
      sels.map(function(s,i){
        var bTxt=s.banko?'[BANKO] ':'';
        return (i+1)+') '+bTxt+s.match+' | '+s.market+': '+s.pick+' @'+fmtOdd(s.odd)+(selectionProposersText(s)?' ('+selectionProposersText(s)+')':'');
      }).join('\n')+
      '\n──────────\n'+
      'Kolon Sayısı: '+totalCols+' · Misli: '+mult+'x\n'+
      'Kupon Bedeli: '+fmtTL(totalStake)+'\n'+
      'Olası Kazanç: Min '+fmtTL(pot.min)+' · Maks '+fmtTL(pot.max);
  }
...
function couponTypeLabel(c){
  if(isSystemCoupon(c)){
    var cfg=parseSystemConfig(c);
    var lvlStr=(cfg&&cfg.levels&&cfg.levels.length)?cfg.levels.join(','):'';
    return 'Sistem '+(lvlStr?('('+lvlStr+')'):'');
  }
  return TYPE_TR[c.type]||'Kupon';
}
...
function renderCoupons(){
  ...
  el.innerHTML=list.map(function(c){
    var isSys=isSystemCoupon(c);
    var st=couponStatus(c),pnl=couponPnl(c);
    var headMeta='',extraPill='';

    if(isSys){
      var ev=evaluateSystemCoupon(c);
      var pot=couponPotentialWin(c);
      extraPill='<span class="pill sys-cols">'+(ev?ev.totalColumns:'–')+' Kolon</span>';
      if(st==='won'){
        headMeta='<span class="muted">'+fmtTL(c.stake)+' · '+(ev?ev.wonColumns:0)+'/'+(ev?ev.totalColumns:0)+' Kolon Tuttu = <b>'+fmtTL(ev?ev.wonPayout:0)+'</b></span>';
      }else if(st==='lost'){
        headMeta='<span class="muted">'+fmtTL(c.stake)+' · '+(ev?ev.totalColumns:0)+' Kolon</span>';
      }else{
        headMeta='<span class="muted">'+fmtTL(c.stake)+' · '+(ev?ev.totalColumns:0)+' Kolon (Maks: <b>'+fmtTL(pot.max)+'</b>)</span>';
      }
    }else{
      var odds=couponOdds(c),pot=c.stake*odds;
      headMeta='<span class="muted">'+fmtTL(c.stake)+' × '+fmtOdd(odds)+' = <b>'+fmtTL(pot)+'</b></span>';
    }

    return '<div class="coupon '+(isSys?'coupon-system':'')+'" data-id="'+c.id+'">'+
      '<div class="chead"><b>'+new Date(c.date+'T12:00').toLocaleDateString('tr-TR',{day:'numeric',month:'short',weekday:'short'})+'</b>'+
      '<span class="pill '+(isSys?'sistem':c.type)+'">'+couponTypeLabel(c)+'</span>'+extraPill+'<span class="pill '+st+'">'+ST_TR[st]+'</span>'+
      '<span class="grow"></span>'+headMeta+'</div>'+
      '<div class="cbody">'+c.selections.map(function(s, idx){
        var bBadge=s.banko?'<span class="leg-banko-tag">B</span>':'';
        return '<div class="legrow" data-idx="'+idx+'" data-sid="'+(s.id||s.propId||s.eventId||('sel_'+idx))+'">'+
        '<div class="lm"><b>'+bBadge+esc(s.match)+'</b><span class="livesc" data-eid="'+(s.eventId||'')+'" data-ko="'+(s.ko!=null?s.ko:'')+'"></span><small>'+esc(s.market)+': <b>'+esc(s.pick)+'</b> @'+fmtOdd(s.odd)+(selectionProposersHtml(s)?' · '+selectionProposersHtml(s):'')+'</small></div>'+
        '<button class="resbtn w '+(s.result==='won'?'on':'')+'" title="Tuttu">✓</button>'+
        '<button class="resbtn l '+(s.result==='lost'?'on':'')+'" title="Yattı">✗</button>'+
        '<button class="resbtn v '+(s.result==='void'?'on':'')+'" title="İade">–</button></div>';
      }).join('')+'</div>'+
    ...
    box.querySelectorAll('[data-act]').forEach(function(b){b.onclick=function(){
      ...
      if(act==='del'){if(confirm('Kupon silinsin mi?'))mutate('delCoupon',{id:c.id});}
        ...
        if(isSys){
          ...
          var ev=evaluateSystemCoupon(c);
          var pot=couponPotentialWin(c);
          var txt='🎲 SİSTEM KUPONU ('+(cfg.levels.join(','))+') – '+new Date(c.date+'T12:00').toLocaleDateString('tr-TR',{day:'numeric',month:'long'})+(st!=='open'?' ['+ST_TR[st]+']':'')+'\n'+
            c.selections.map(function(s,i){
              var bTxt=s.banko?'[B] ':'';
              return (i+1)+') '+bTxt+s.match+' | '+s.market+': '+s.pick+' @'+fmtOdd(s.odd)+(selectionProposersText(s)?' ('+selectionProposersText(s)+')':'')+(s.result==='won'?' ✓':s.result==='lost'?' ✗':'');
            }).join('\n')+
            '\n──────────\n'+
            'Kolon: '+(ev?ev.totalColumns:'–')+' · Misli: '+(ev?ev.multiplier:1)+'x\n'+
            'Yatırım: '+fmtTL(c.stake)+'\n'+
            (st==='won'?('KAZANÇ ('+(ev?ev.wonColumns:0)+' Kolon): '+fmtTL(ev?ev.wonPayout:0)):('Olası Kazanç: Min '+fmtTL(pot.min)+' · Maks '+fmtTL(pot.max)));
...
function renderStats(){
  ...
  if(typeof Chart==='undefined'){
    setTimeout(function(){if(typeof Chart!=='undefined')renderStats();},150);
    return;
  }
...
function renderAgentReport(data) {
  ...
  if (isOrtakBos) {
    ...
    ort.forEach(function(b) {
      ...
      var altParts = [];
      if (b.kickoff_trt) altParts.push(esc(b.kickoff_trt));
      altParts.push((b.kisa || b.secim) + ' <b>' + oddStr + '</b>');
      if (b.puan != null) {
        var pStr = '+' + Number(b.puan).toFixed(1).replace('.', ',') + 'p';
  ...
  (d.bacaklar || []).forEach(function(bc) {
    ...
    var timeTag = bc.kontrol_saati
      ...
      : '';
    var pencereTag = (bc.kod === 'hareket' && d.pencere && d.pencere.metin)
      ? '<span class="bacak-pencere-badge">' + esc(d.pencere.metin) + '</span>'
      : '';
    ...
    if (bc.kod === 'futbol' || bc.no === 2) {
      if (bc.oyuncu_notu && String(bc.oyuncu_notu).trim() !== '') {
    ...
    if (bc.kod === 'hareket' || bc.no === 3) {
      ...
      if (HT && ((HT.dusenler && HT.dusenler.length) || (HT.yukselenler && HT.yukselenler.length))) {
        ...
        if (HT.aciklama) {
          html += '<p class="agent-ht-gosterge">' + esc(HT.aciklama) + '</p>';
        }
        if (HT.tablo_filtresi) {
          html += '<p class="agent-ht-gosterge">' + esc(HT.tablo_filtresi) + '</p>';
        }
      ...
      if (acikHareket.length > 0) {
        ...
        var topRanks = [
          { title: "Pencerenin En İyi Hareketi", rankClass: "rank-1", icon: "trophy", item: acikHareket[0] },
          { title: "Pencerenin En İyi İkinci Hareketi", rankClass: "rank-2", icon: "medal", item: acikHareket[1] }
        ];
        topRanks.forEach(function(r) {
          ...
          var timePart = x.kickoff_trt ? ' <span class="atm-time">· ' + esc(x.kickoff_trt) + '</span>' : '';
          html += '<div class="atm-item ' + r.rankClass + '">' +
            '<div class="atm-badge"><ion-icon name="' + r.icon + '"></ion-icon> ' + esc(r.title) + '</div>' +
            '<div class="atm-row">' +
              '<span class="atm-match"><b>' + esc(x.mac) + '</b>' + timePart + '</span>' +
      ...
      if (d.hareket_yorum) {
        ...
        if (hy.gunun_en_iyisi) {
          html += '<div class="amb-row highlight">' +
            '<strong>Pencerenin En İyi Hareketi:</strong>' +
...
function mapSupabaseAiPrediction(row) {
  ...
  return {
    ...
    initialOdd: Number(row.initial_odd != null ? row.initial_odd : row.odd) || 1.8,
  ...
  var updated = list.map(function(p) {
    ...
    if (liveEv && liveEv.ms && liveEv.ms.o && liveEv.ms.o.length) {
      ...
      if (matchedOdd) {
        ...
        if (p.initialOdd == null) p.initialOdd = p.odd;
        p.odd = matchedOdd;
        p.mktI = matchedMktI;
        p.no = matchedNo;
      }
...
function aiCardHtml(p, idx, total) {
  ...
  var curOdd = Number(p.odd) || 1.8;
  var initOdd = Number(p.initialOdd != null ? p.initialOdd : p.odd) || curOdd;
  var diff = Math.round((curOdd - initOdd) * 100) / 100;
  var diffHtml = (Math.abs(diff) >= 0.01)
    ? '<span style="font-size:11px!important;color:#ffb92e!important;font-weight:700!important;margin-right:6px!important;" title="İlk oran: @' + fmtOdd(initOdd) + '">İlk: @' + fmtOdd(initOdd) + (curOdd > initOdd ? ' ↗' : ' ↘') + '</span>'
    : '';
